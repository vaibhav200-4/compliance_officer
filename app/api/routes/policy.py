    ...
    try:
        ...
        if file_path and file_path.exists():
            ...
            try:
                ...
                ingest_result = pipeline.ingest(str(file_path))
                document_ids = ingest_result.get("document_ids") or []
                document_id = document_ids[0] if document_ids else None
                logger.success(
                    f"[{job_id}] Policy ingested successfully. "
        ...
        # Scoping retrieval to this job's document_id (when a fresh
        # policy was just uploaded) stops chunks from a previously
        # ingested policy in the same Pinecone namespace from being
        # retrieved during this analysis run.
        orchestrator = ComplianceOrchestrator(knowledge_base=kb, document_id=document_id)
